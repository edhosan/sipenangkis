...
<div class="tab-pane active">
  <div class="box box-primary">
    <div class="box-header with-border">
      <h3 class="box-title">Form Proses Intervensi Kelompok</h3>
    </div>
    <form role="form" @submit.prevent="simpan">
      ...
      <div class="box-footer">
        ...
        <a v-link="{ path: '/pengajuan/kelompok' }" class="btn btn-warning">Batal</a>
      </div>
    </form>
  </div>
...
export default {
	...
	ready(){
		...
		if(this.isAction == 'add'){	     
	      this.fetchKelompok(this.$route.params.id)
	    }else{
	...
	methods:{
	    ...
	    fetchKelompok: function(penerima_id){
	      this.$Progress.start()
	      this.$http.get(this.$root.$config.API + '/api/kelompok/'+penerima_id+'/edit').then((response)=>{
	        this.$set('nama_penerima',response.data.kelompok.name)
	        this.$Progress.finish()
	      },(error)=>{
	        console.log(error)
	        this.$Progress.fail()
	    ...
	    fetchIntervensi: function(program_id){
	      this.$Progress.start()
	      this.$http.get(this.$root.$config.API + '/api/intervensi/'+program_id+'/3/intervensi_program').then((response)=>{
	    ...
	    simpan: function(){
	      ...
	      if(this.isAction=='add'){
	          ...
	          url = url + '/api/program_intervensi/'+this.$route.params.id+'/store_kelompok'
	      }else{
	          url = url + '/api/program_intervensi/update_intervensi_kelompok'
	      }
	    ...
	    fetchData: function(id){
	      this.$Progress.start()
	      this.$http.get(this.$root.$config.API + '/api/program_intervensi/'+id+'/edit_kelompok').then((response)=>{
	        this.$set('nama_penerima',response.data.desa.name)
