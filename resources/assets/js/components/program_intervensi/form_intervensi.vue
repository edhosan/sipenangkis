  ...
  <div class="box box-primary">
    ...
    <form role="form" @submit.prevent="simpan">
      ...
      <div class="box-footer">
        ...
        <a v-link="{ path: '/pengajuan/keluarga' }" class="btn btn-warning">Batal</a>
      </div>
    </form>
  </div>
...
export default {
  ...
  methods: {
    ...
    fetchKepalaKeluarga: function(penerima_id){
      this.$Progress.start()
      this.$http.get(this.$root.$config.API + '/api/keluarga/'+this.$route.params.id+'/kepala_keluarga').then((response)=>{
        this.$set('nama_penerima',response.data.nama)
        this.$Progress.finish()
      },(error)=>{
        console.log(error)
        this.$Progress.fail()
