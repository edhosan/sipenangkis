  ...
  <div class="tab-pane active"> 
    <div class="box box-warning">
      <div class="box-header with-border">
        <h3 class="box-title">Wilayah Penerima</h3>
      </div>
      <div class="box-body">
    ...
    <div class="box box-primary">
       ...
       <a v-link="{ path: '/pengajuan/form_intervensi_kawasan/add/'+$route.params.id }" class="btn btn-primary"><i class="fa fa-random"></i> Proses Pemberian Intervensi</a>
  </div>
...
export default {
  data(){
    return{
          kecamatan:[],
          desa:[],
          url_penerima: this.$root.$config.API + '/api/pembaharuan/index',
          fields_penerima: [
  ...
  methods:{
    fetchData: function(id){
      this.$Progress.start()
        this.$http.get(this.$root.$config.API + '/api/desa/'+id+'/detail').then((response)=>{
  ...
  events:{
      'vuetable:action': function(action, data) {
           if (action == 'edit-item') {
             this.$router.go({path:'/pengajuan/form_intervensi_kawasan/edit/'+data.id})
