...
<div class="tab-pane active">
  <div class="box box-primary">
    <div class="box-header with-border">
      <h3 class="box-title">Form Proses Intervensi Kawasan</h3>
    </div>
    <form role="form" @submit.prevent="simpan">
      ...
      <div class="box-footer">
        ...
        <a v-link="{ path: '/pengajuan/kawasan' }" class="btn btn-warning">Batal</a>
      </div>
    </form>
  </div>
...
export default {
	...
	ready(){
		...
		if(this.isAction == 'add'){	     
	      this.fetchKawasan(this.$route.params.id)
	    }else{
	...
	methods:{
	    ...
	    fetchKawasan: function(penerima_id){
	      this.$Progress.start()
	      this.$http.get(this.$root.$config.API + '/api/desa/'+penerima_id+'/detail').then((response)=>{
	    ...
	    fetchIntervensi: function(program_id){
	      this.$Progress.start()
	      this.$http.get(this.$root.$config.API + '/api/intervensi/'+program_id+'/2/intervensi_program').then((response)=>{
	    ...
	    simpan: function(){
	      ...
	      if(this.isAction=='add'){
	          ...
	          url = url + '/api/program_intervensi/'+this.$route.params.id+'/store_kawasan'
	      }else{
	          url = url + '/api/program_intervensi/update_intervensi_kawasan'
	      }
	    ...
	    fetchData: function(id){
	      this.$Progress.start()
	      this.$http.get(this.$root.$config.API + '/api/program_intervensi/'+id+'/edit_kawasan').then((response)=>{
	        this.$set('nama_penerima',response.data.desa.name)
