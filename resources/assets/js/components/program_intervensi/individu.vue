	  ...
	  <div class="col-md-12" v-if="isAction == 'list'">
	    <div class="box box-warning">
	      <div class="box-header with-border">
	        <h3 class="box-title">Daftar Penerima Manfaat Individu</h3>
	      </div>
	      <!-- /.box-header -->
	      <div class="box-body">
	        <div class="row">
...
export default{
	data(){
		return{
		  isAction: 'list',
		  per_page: 10,
	      url: this.$root.$config.API + '/api/program_intervensi/index_individu',
	      fields:[
	        { name: '__sequence', title:'No.' },
	        { name:'id', sortField: 'id', visible: false },
	        { name:'nik', sortField: 'nik', title:'NIK' },
	      ...
	      filterBy:[
	        { val:'m_penerima_manfaat_keluarga.id', label:'Id' },
	        { val:'m_penerima_manfaat_keluarga.nik', label:'NIK' },
	...
	events:{
    	'vuetable:action': function(action, data) {
        	this.$Progress.start()
         	if (action == 'edit-item') {
           		this.$router.go({path:'/pengajuan/detail_individu/'+data.id})
         	}
    	}
    }
}
