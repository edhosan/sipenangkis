  ...
  <div class="row">
    <div class="col-md-12" v-if="isAction == 'list'">
      <div class="box">
        <div class="box-header with-border">
          <h3 class="box-title">Daftar Stakeholders</h3>
        </div>
        <!-- /.box-header -->
        <div class="box-body">
    ...
    <div class="col-md-12" v-if="isAction == 'add' || isAction == 'edit'">
      <div class="box box-primary">
        <div class="box-header with-border">
          <h3 class="box-title">Form Stakeholders</h3>
        </div>
        <!-- /.box-header -->
        <!-- form start -->
          ...
          <div class="box-body">
            <div class="col-md-12">
              <div class="form-group" v-bind:class="{'has-error':!validation.name}">
                ...
                <input type="text" class="form-control" id="frmNama" placeholder="Nama" v-model="stakeholder.name" :disabled="isFreeze">
                <span class="help-block" v-if="!validation.name">Nama stakeholder harus diisi.</span>
              </div>
            </div>
          <!-- /.box-body -->
          </div>
...
export default {
  data () {
    return {
      stakeholder:{ id: null, name: '', program:[]},
      list:[],
      offset: 4,
      per_page: 10,
      pagination: {
  ...
  computed: {
    ...
    validation: function(){
      return{
        name:!!this.stakeholder.name.trim()
      }
    },
  ...
  methods: {
    fetchData: function(page) {
      this.$Progress.start()
      var url = this.$root.$config.API + '/api/stakeholder/index?page='+page+'&per_page='+this.per_page+'&sort=id|asc'
    ...
    entryItem: function(){
      this.isAction = 'add'
      this.stakeholder = {id:null,name:'',desa:[]}
      this.isFreeze = false
    },
    ...
    simpan: function(){
      ...
      var data = this.stakeholder
      var url = this.$root.$config.API

      if(this.isAction=='add'){
          url = url + '/api/stakeholder/store'
      }else{
          url = url + '/api/stakeholder/'+this.stakeholder.id+'/update'
      }
    ...
    deleteItem: function(item){
      this.$Progress.start()
      this.$http.get(this.$root.$config.API + '/api/stakeholder/'+item.id+'/delete').then((response)=>{
  ...
  route: {
      activate: function(t) {
          this.fetchData(this.pagination.current_page)
          this.$parent.$parent.$data.title = 'Stakeholders';
          t.next();
      }
  }
}
